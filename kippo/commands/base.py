...
class command_uptime(HoneyPotCommand):
    def call(self):
        ...
        self.writeln(
            f" {time.strftime('%H:%M:%S')} up {utils.uptime(self.honeypot.uptime())},  1 user,  load average: 0.00, 0.00, 0.00"
        )
...
class command_uname(HoneyPotCommand):
    def call(self):
        if len(self.args) and self.args[0].strip() in ('-a', '--all'):
            self.writeln(
                f'Linux {self.honeypot.hostname} 2.6.26-2-686 #1 SMP Wed Nov 4 20:45:37 UTC 2009 i686 GNU/Linux'
            )
        else:
            self.writeln('Linux')
...
class command_shutdown(HoneyPotCommand):
    def start(self):
        if len(self.args) and self.args[0].strip().count('--help'):
            ...
            self.display_shutdown_message('The system is going down for maintenance NOW!')
        elif len(self.args) > 1 and self.args[0].strip().count('-r') \
                    and self.args[1].strip().count('now'):
            self.display_shutdown_message('The system is going down for reboot NOW!')
        else:
    ...
    def display_shutdown_message(self, arg0):
        self.nextLine()
        self.writeln(
            f'Broadcast message from root@{self.honeypot.hostname} (pts/0) ({time.ctime()}):'
        )
        self.nextLine()
        self.writeln(arg0)
...
class command_history(HoneyPotCommand):
    def call(self):
        if len(self.args) and self.args[0] == '-c':
            ...
            return
        for count, l in enumerate(self.honeypot.historyLines, start=1):
            self.writeln(f' {str(count).rjust(4)}  {l}')
    ...
    def call(self):
        ...
        for arg in self.args[1:]:
            ...
            if not self.fs.exists(path):
                self.writeln(f'chmod: cannot access {arg}: No such file or directory')
