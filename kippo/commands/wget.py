...
class command_wget(HoneyPotCommand):
    ...
    def download(self, url, fakeoutfile, outputfile, *args, **kwargs):
        try:
            ...
            self.writeln(f'{url}: Unsupported scheme.')
            self.exit()
            return None

        self.writeln(f"--{time.strftime('%Y-%m-%d %H:%M:%S')}--  {url}")
...
class HTTPProgressDownloader(client.HTTPDownloader):
    ...
    def gotHeaders(self, headers):
        if self.status == '200':
            ...
            if self.totallength > 0:
                ...
                self.wget.writeln(f'Length: unspecified [{self.contenttype}]')
            if 0 < self.wget.limit_size < self.totallength:
                f'Not saving URL ({self.wget.url}) due to file size limit'
                self.fileName = os.path.devnull
                self.nomore = True
            else:
                msg = f'Saving URL ({self.wget.url}) to {self.fileName}'
                self.wget.honeypot.logDispatch(msg)
                msg
            print
            self.wget.writeln(f'Saving to: `{self.fakeoutfile}')
    ...
    def pagePart(self, data):
        if self.status == '200':
            ...
            if not self.nomore and 0 < self.wget.limit_size < self.currentlength:
                self.handle_file_size_limit_reached(data)
    ...
    def handle_file_size_limit_reached(self, data):
        print
        'File limit reached, not saving any more data!'
        self.nomore = True
        self.file.close()
        self.fileName = os.path.devnull
