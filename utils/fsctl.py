...
A_NAME, A_TYPE, A_UID, A_GID, A_SIZE, A_MODE, \
    A_CTIME, A_CONTENTS, A_TARGET, A_REALFILE = range(10)
...
def resolve_reference(pwd, relativeReference):
    ...
    return os.path.normpath(tempPath)
...
class fseditCmd(cmd.Cmd):
    ...
    def __init__(self, pickle_file_path):
        ...
        if not os.path.isfile(pickle_file_path):
            print
            f"File {pickle_file_path} does not exist."
            sys.exit(1)
        ...
        try:
            ...
            print
            f"Unable to open file {pickle_file_path}"
            sys.exit(1)
    ...
    def do_ls(self, args):
        ...
        path = resolve_reference(self.pwd, args) if len(args) else self.pwd
    ...
    def update_pwd(self, directory):
        self.pwd = directory
        self.prompt = f"{self.fs_name}:{self.pwd}$ "
    ...
    def do_cd(self, args):
        ...
        if not len(arguments):
            ...
            if exists(self.fs, target_dir) == False:
                ...
                print
                f"cd: {target_dir}: Not a directory"
    ...
    def mkfile(self, args, file_type):
        ...
        size = 4096 if len(args) == 1 else args[1]
    ...
    def do_rm(self, arguments):
        ...
        if exists(self.fs, target_path) == False:
            print
            "File \'%s\' doesn\'t exist" % (target_path,)
            return
        ...
        if target_object[A_TYPE] == T_DIR and args[0] != "-r":
            print
            f"rm: cannot remove '{target_path}': Is a directory"
            return

        self._extracted_from_do_rmdir_40(target_path, target_object)
        print
        f"Deleted {target_path}"
    ...
    def _extracted_from_do_rmdir_40(self, target_path, target_object):
    ...
    def do_mv(self, arguments):
        ...
        if not exists(self.fs, src):
            print
            "Source file \'%s\' does not exist!" % src
            return
        ...
        if exists(self.fs, dst) and is_directory(self.fs, dst):
            ...
            dstname = dst.split('/')[-1]
        if exists(self.fs, f'{dstparent}/{dstname}'):
            print
            f"A file already exists at {dst}!"
            return
        if not exists(self.fs, dstparent):
            print
            "Destination directory \'%s\' doesn\'t exist!" % dst
            return
        if src == self.pwd:
            self.do_cd("..")
        self.move_or_copy_filesystem_object(dstparent, srcl, dstname)
        srcparentl[A_CONTENTS].remove(srcl)
        self.save_pickle()
        print
        f'File moved from /{src} to /{dst}'
    ...
    def do_cp(self, arguments):
        ...
        if exists(self.fs, f'{dstparent}/{dstname}'):
            print
            f'A file already exists at {dstparent}/{dstname}!'
            return
        if not exists(self.fs, dstparent):
            print
            "Destination directory %s doesn\'t exist!" % (dstparent,)
            return
        self.move_or_copy_filesystem_object(dstparent, srcl, dstname)
        self.save_pickle()
        print
        f'File copied from /{src} to /{dstparent}/{dstname}'

    # TODO Rename this here and in `do_rm`, `do_rmdir`, `do_mv`, `do_cp` and `do_file`
    def do_file(self, args):
        ...
        if not exists(self.fs, target_path):
            print
            f"File '{target_path}' doesn't exist."
            return
        ...
        print
        f"{target_path} is a {msg}"

    def move_or_copy_filesystem_object(self, dstparent, srcl, dstname):
    ...
    def help_about(self):
        print
        var = "Kippo stores information about its file systems in a " + \
...
if __name__ == '__main__':
    if len(sys.argv) != 2:
        print
        f"Usage: {os.path.basename(sys.argv[0])} <fs.pickle>"
        sys.exit(1)
